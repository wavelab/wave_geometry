...
decltype(auto) evalImpl(expr<Sum>, const TranslationBase<Lhs> &lhs, const Zero<Rhs> &) {
    return lhs.derived();
}

template <typename Lhs, typename Rhs>
decltype(auto) evalImpl(expr<Sum>, const Zero<Lhs> &, const TranslationBase<Rhs> &rhs) {
    return rhs.derived();
}
